...
describe('MIP member publication lifecycle', () => {
  ...
  it('uses status transitions and never presents member-side deletion', () => {
    ...
    expect(templates).toContain('下架合作卡')
    expect(templates).toContain('下架案例')
    expect(templates).not.toContain('删除合作卡')
    expect(templates).not.toContain('删除案例')
  })
