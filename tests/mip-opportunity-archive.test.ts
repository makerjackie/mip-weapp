...
describe('MIP opportunity archive contract', () => {
  it('keeps archived drafts out of owner reads and editing', () => {
    ...
    expect(source).toContain('const OWNER_EDITABLE_OPPORTUNITY_STATUSES = new Set([\'DRAFT\', \'PUBLISHED\', \'UNPUBLISHED\'])')
    expect(source).toContain('!(status === \'UNPUBLISHED\' && moderatedByUserId)')
    expect(source).toContain('if (!canOwnerEditOpportunity(existing.status, existing.moderated_by_user_id)) throw new Error(\'FORBIDDEN\')')
  })
})
