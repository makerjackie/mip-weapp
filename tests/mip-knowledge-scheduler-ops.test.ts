...
describe('MIP knowledge scheduler operations', () => {
  ...
  it('exposes stable secret initialization, admin injection, and dedicated commands', () => {
    ...
    expect(example).not.toContain('MIP_KNOWLEDGE_SCHEDULER_ROLE_NAME=MIPKnowledgeSchedulerRole')
    expect(example).not.toContain('MIP_KNOWLEDGE_SCHEDULER_HMAC_SECRET=')
  })
})
