...
describe('MIP event discovery filter client', () => {
  ...
  it('renders catalog filters, real event tags, and a rolling calendar that reaches long-lived demo events', () => {
    ...
    const card = readFileSync(new URL('../src/components/mip-activity-card/index.wxml', import.meta.url), 'utf8')
    const cardModel = readFileSync(new URL('../src/components/mip-activity-card/model.ts', import.meta.url), 'utf8')
    expect(page).toContain('calendarMaxDate: rollingCalendarBoundary(10)')
    expect(page).toContain('feed.items.map(presentEventCard)')
    expect(cardModel).toContain('startsText: formatChineseDateTime(event.startsAt)')
    ...
    expect(view).toContain('id="events-clear-filters"')
    expect(view).toContain('bind:tap="clearDiscoveryFilters">清空筛选')
    expect(view).toContain('bind:tap="confirmDiscoveryFilters">确定筛选')
    expect(view).toContain('<mip-activity-card')
