...
describe('existing core-function configuration updates', () => {
  ...
  it('keeps the runtime order config, readback, code, readback and masks raw errors', () => {
    ...
    const publicNetworkWrite = source.indexOf('await ensureAdminWebPublicNetwork(spec.name)')
    ...
    expect(source).toContain('await ensureAdminWebPublicNetwork(spec.name)')
