...
describe('existing core-function configuration updates', () => {
  ...
  it('keeps the runtime order config, readback, code, readback and masks raw errors', () => {
    ...
    const publicNetworkWrite = source.indexOf('await ensureWechatApiPublicNetwork(spec.name)')
    const codeBaseline = source.indexOf('const codeBaselineSha256 = codeBaselineDetail')
    const codeWrite = source.indexOf('callCloudbase(root, \'manageFunctions\', codeUpdate')
    ...
    const activeCheck = source.indexOf('const active = value?.Status === \'Active\'', boundedWait)
    ...
    expect(configurationReadback).toBeLessThan(publicNetworkWrite)
    expect(publicNetworkWrite).toBeLessThan(codeBaseline)
    expect(codeBaseline).toBeLessThan(codeWrite)
    ...
    expect(source.slice(boundedWait)).toContain('existingFunctionCodeSha256(functionName) !== baselineSha256')
    expect(source.slice(boundedWait)).toContain('if (active && phase === \'code\')')
    expect(source).toContain('action: \'GetFunctionAddress\'')
    expect(source).toContain('if (managementRequestRejected(codeUpdateResponse))')
    expect(source).toContain('path.join(stagingRoot, spec.name, \'mip-deployment-marker.json\')')
    expect(source).toContain('await ensureWechatApiPublicNetwork(spec.name)')
    expect(source).toContain('PublicNetStatus: \'ENABLE\'')
  })
})
