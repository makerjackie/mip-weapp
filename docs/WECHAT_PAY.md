...
部署三个可选支付函数前必须先部署 16 个核心函数和对应 `mip_*` schema。支付函数使用 Node.js 20.19、可信 AppID、专用回调函数名和独立退款 worker HMAC；商户号、证书、签名材料和内部密钥只进入函数配置，日志不得输出 OpenID、金额密钥或支付凭证。团队赛季与排行榜只按当前有效会员权益开放；它们读取 ledger 已确认的权益和服务端成长流水，不读取客户端支付结果或客户端分数。
