...
| CloudBase / 支付模式 | `.env.local` + 可选 `.env.secrets.local` + `src/config/runtime.ts` |
...
脚本可重复执行。密钥不会写入 Git 跟踪文件。使用 `pnpm env:compact` 可将稳定密钥和服务端凭证移入 `.env.secrets.local`；未知用户改动会与现有 `.env.local` 合并，而不是整个覆盖。
