...
| 健康调用报 `[Invoke] Cam authentication failed`，但部署或小程序业务调用正常 | 先排查 keep-alive MCP 缓存的授权状态。使用本次操作原有授权模式重启 `restartCloudbaseMcp`（见下方命令），然后重跑完整 `cloud:verify`。只有健康和配置回读通过才算恢复；不要直接扩大 CAM 权限，也不要自动重试可能已成功的业务写请求。 |
...
## CloudBase 常驻进程授权状态恢复

已授权使用本机 Device Flow、且出现上述健康调用鉴权失败时，在仓库根目录使用项目要求的 Node 版本执行：

```bash
CLOUDBASE_AUTH_MODE=local node --input-type=module -e "import { restartCloudbaseMcp } from './scripts/lib/cloudbase-mcp-runner.mjs'; restartCloudbaseMcp(process.cwd())"
CLOUDBASE_AUTH_MODE=local pnpm cloud:verify -- --confirm-env=<本地配置中的环境ID>
```

重启会重建本项目使用的 MCP 常驻连接，不创建新凭证或扩大权限。2026-09-16 的同类故障在重启后，同一健康请求恢复 `ok=true`、`persistence=cloudbase-mysql`。这只能证明当次故障与常驻授权状态有关；若仍失败，应继续检查凭证有效期及调用权限，不把所有 CAM 错误归因于缓存。
