...
条目数只记录本次开发环境快照，不是固定业务口径。本次登录后线上验收时 BFF 开放 33 条受审只读 action；当前已部署版本扩展为 68 条查询和 59 条受审写 action。除一级页面外，本次还真实读取了用户详情、活动详情/洞察/报名名单、订单详情/支付尝试、消息活动详情和知识内容详情/采集计划。扩展版本已完成公开域名 200、未登录会话和静态资源指纹回读，但没有在本次证据中补造登录后读写结论；未展示的 action 仍由 fail-closed 合同测试覆盖。
