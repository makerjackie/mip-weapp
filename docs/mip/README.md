...
当前工程声明 110 条小程序路由、187 个渠道中立管理 operation（80 查询、107 写）、16 个核心 `mip-*` 云函数和 56 个锁定迁移。2026-08-28 共享云环境已验证 56 个迁移与 122 张 MIP runtime 表的精确权限；16 个核心函数曾形成完整健康快照，但本轮代码部署回读因 CloudBase `InsufficientBalance` 未收敛，不能把当前工作区桥接代码写成已部署。2026-08-27 微信开发者工具报告曾在 375px 实测窗口通过当时的 108/108 路由、6/6 代表状态和 6/6 交互旅程；最新一次已访问当前 110/110 路由，但 40 条受后端不可用影响、12 条保留外部待验，因此不是 110/110 通过。档案、名片、名片设置、徽章目录和徽章详情另有 5/5 `ready` 定向证据和 8 张连续视口截图。真机能力、Mac/Windows 微信客户端和正式外部配置继续作为明确验收边界。
