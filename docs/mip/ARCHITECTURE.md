...
MIP 是一个原生微信小程序、16 个核心 CloudBase 函数和三个可选支付函数组成的会员与协作平台。当前工程声明 83 条小程序路由，数据库 lock 固定 34 个追加迁移。短期复用共享 CloudBase，业务数据、函数名和对象 key 使用 MIP 专属边界；正式 AppID 上线时迁移到空的独立 CloudBase/MySQL 环境。当前 schema 不支持在同一数据库中以相同主键并存两个 AppID 副本。
...
| game | 赛季、团队成员历史、每周赛况、团队/个人排行快照、队伍大本营和周赛游戏币事件 | 客户端不能传分数或游戏币；只对当前有效会员开放；正式规则与视觉可替换 |
