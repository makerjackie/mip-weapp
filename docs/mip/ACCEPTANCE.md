...
2026-08-28 当前工作区通过完整静态门禁：客户端 221 个测试文件、1262 项测试；服务端核心函数 225 个测试文件、1420 项测试；源码合同为 139 个页面视图、16 个核心函数、56 个锁定迁移。架构、共享数据库隔离、安全、类型、lint、样式、构建、包体和文档链接同时通过，ESLint 为 0 warning、0 error。静态门禁不等于运行时、云端或真机证据。
...
以下事项即使本地测试通过，仍保留 `external-wait`：正式 AppID、支付商户和回调、正式协议正文、正式标签/城市和 AME 配置、通知模板、AI provider、活动介绍图片与任务模板的真机选择/上传/内容安全/临时 URL/相册保存、正式勋章目录图片、正式游戏化规则与队伍大本营视觉、手机号授权/换绑、扫码签到、地图/日历和真实支付。当前 56 个数据库迁移、122 张 MIP runtime 表的精确权限以及最终工作区代码对应的 16 个核心函数已经在共享 CloudBase 验证。
...
- 当前 Web BFF 开放 62 条经过生成契约复核的只读 action，10 个一级页面与用户、活动、订单、消息、知识库、机会 6 类详情已接入真实查询、筛选和适用的游标分页。补录会员、克隆活动、发布活动提醒和提交退款 4 个写操作表单已经部署，并由精确白名单、一次性 nonce、业务幂等键和 capability 共同约束；新增页面与 action 已通过本地契约和响应式验证，仍需补录登录后线上读回证据。未执行真实业务写入，其他 CRUD、上传和导出仍未完成。
