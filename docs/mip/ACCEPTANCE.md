...
2026-08-28 当前工作区通过完整静态门禁：客户端 221 个测试文件、1262 项测试；服务端核心函数 225 个测试文件、1415 项测试；源码合同为 139 个页面视图、16 个核心函数、56 个锁定迁移。架构、共享数据库隔离、安全、类型、lint、样式、构建、包体和文档链接同时通过，ESLint 为 0 warning、0 error。静态门禁不等于运行时、云端或真机证据。
...
- 当前 Web BFF 开放 33 条经过生成契约复核的只读 action，7 个一级页面与用户、活动、订单、消息、知识库详情已接入真实查询、筛选和适用的游标分页。另有 4 条具备持久业务幂等保护的 mutation 通过精确白名单和一次性 nonce 防重放开放；网页写操作表单、其余 CRUD、上传和导出仍未完成。
