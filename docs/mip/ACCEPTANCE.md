...
- 当前 Web BFF 开放 33 条经过生成契约复核的只读 action，7 个一级页面与用户、活动、订单、消息、知识库详情已接入真实查询、筛选和适用的游标分页。补录会员、克隆活动、发布活动提醒和提交退款 4 个写操作表单已经部署，并由精确白名单、一次性 nonce、业务幂等键和 capability 共同约束；未执行真实业务写入，其他 CRUD、上传和导出仍未完成。
