...
- 当前 Web BFF 开放 68 条经过生成契约复核的查询 action 与 59 条受审写 action；11 个一级页面及用户、活动、订单、任务、任务完成记录、消息、知识库、机会 8 类详情已接入真实查询、筛选和适用的游标分页。写操作由精确字段白名单、一次性 nonce、capability 与作用域共同约束，其中 17 条具有领域持久幂等；其余操作不自动重试。新增页面与 action 已通过本地契约及 390px/1280px 响应式验证，仍需重新部署和登录后线上读写证据。媒体上传、用户/订单敏感导出和正式外部能力仍未完成。
