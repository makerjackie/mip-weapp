...
2026-08-27 当前 `main` 在 Node 22 下通过完整门禁：客户端 210 个测试文件、1220 项测试；服务端 220 个测试文件、1389 项测试；源码合同为 133 个页面视图、16 个核心函数、52 个锁定迁移。架构、共享数据库隔离、安全、类型、样式、构建、包体和文档链接同时通过；ESLint 仅保留 16 条既有顺序 warning，没有 error。
...
`.tmp/runtime/report.json` 生成于 2026-08-25，只遍历当时的 95 条路由：30 条通过、27 条 `external-wait`、38 条失败。当前配置已增长为 108 条路由和 6 个交互旅程，该文件明确降级为历史报告，不再代表当前运行结论。重新运行时必须冻结代码写入、关闭热重载、单次构建/编译并校验实际 `page.path` 与唯一根节点，避免开发者工具 renderer/Automator 竞态产生伪截图。

当前微信身份已经完成协议、档案和手机号绑定，并由精确档案初始化为唯一 `PLATFORM_OWNER`；手机号原值不写入仓库。`.tmp/bootstrap-owner-result.json` 记录 `ownerCount=1`、`role=PLATFORM_OWNER`、`scope=PLATFORM`。管理仪表盘 375px 真实数据正常态已有证据；旧 1024px 截图暴露指标卡过窄并已触发代码修复，但修复后的 960px+ 截图仍待取得。Owner 身份不等于有效付费玩家，玩家专属路由仍须合法 TEST 权益或另一测试账号。

开发用目录以及 6 个演示用户、3 个测试会员订单与权益、3 场 2030 活动、8 条报名、3 条机会、6 张合作卡和 3 条超级案例已通过 `pnpm seed:demo` 写入当前 AppID 的 MIP 表。公告、知识、消息、任务、成长、赛季、队伍、排行和盲盒夹具也已写入并完成固定计数复核；全部使用固定 ID 和版本化清单，没有修改非 MIP 表。Banner 正常态仍需要真实云媒体素材，异常中心只使用可证明的失败投递事实；完成当前 108 路由复验前不把夹具存在等同于运行验收通过。
...
当前云端证据已经包含 52 个迁移版本、121 张表的精确授权回读、MIP 对象清单、16 个最终代码核心函数清单、VPC/环境配置、专用 runtime MySQL 账号健康检查、客户端调用规则和高频 timer 缺失检查。旧 95 路由报告已经失效；108 路由的代表性手机/宽屏复验、扩展 Demo 正常态、真机能力和正式外部配置仍待收口。演示数据只允许写入当前 AppID 的 `mip_*` 表，不得写入共享环境中的旧项目表。API Key 为 `READY` 或开发者工具可见均不能替代这些证据。
