...
> 2026-08-27 外部 GitHub 已追加后台 PRD V0.5 和评审纪要。本文仍保留 WorkBuddy V0.4 的 110 条逐项证据；V0.5 差异及冲突处理见 [MIP 后台 PRD V0.5 对齐记录](ADMIN_PRD_V05_RECONCILIATION.md)。
...
| ID | 需求点 | Operation | Page | Test | 判定与缺口 |
| --- | --- | --- | --- | --- | --- |
| WB-LIVE-038 | Banner 模块 | `B:list/get/save/changeStatus/move/delete` | `banners`、`banner-editor` | BN | `覆盖`：图片、跳转、窗口、排序、启停、软删除和版本校验均有独立合同；生产 Web 已完成图片上传、以 `INACTIVE` 保存及软删除，未执行启用发布。 |
...
当前仓库有 49 个已注册管理路由；`src/modules/mip-admin/generated/admin-operation-contract.json` 声明 187 个 `mip-admin-api` operation，其中 13 个任务、7 个 Banner、20 个游戏管理 action 和 1 个媒体上传 action 分别通过窄 adapter 调用独立 `mip-tasks-api`、`mip-banners-api`、`mip-game-api`、`mip-media-api`。数量不能直接证明 110 项需求已完成：同一 operation 会服务多行需求，部分底层事实也没有对应管理入口。

2026-08-28 的[当前生产 React 线上验收](evidence/admin-web-live-2026-08-28-react/README.md)已完成 14/14 一级页面登录态真实读取；写入证据覆盖 Banner 图片上传、以 `INACTIVE` 保存和软删除，以及不含手机号、结果 0 行的导出生成、完整性校验和一次性消费。该范围不证明全部 80 条受审 mutation、非空或含手机号导出、Banner 启用发布、正式支付、真机能力或外部 provider 送达。
...
5. 运行验收：49 个管理路由已进入 `.tmp/runtime-evidence/2026-08-28-final-r6/report.json` 的 110/110 路由 375px 全量通过集合；6/6 代表状态、6/6 交互旅程、运行时与 IDE 编译诊断也全部通过。1024px 管理端代表页已有真实目录数据证据；真机支付、扫码、媒体、手机号、订阅消息、外部 provider 和 Mac/Windows 微信客户端继续保留独立边界。活动本地草稿恢复属于可选体验增强，不是当前领域模型缺口。
