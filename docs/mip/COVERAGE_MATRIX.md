...
本矩阵是完整实现的逐项验收入口。状态只使用四个词：`implemented-local` 表示当前代码、迁移和聚焦测试形成了本地证据；`partial-local` 表示仍缺行为、字段、状态或入口；`external-wait` 表示源码链路存在，但能力本身仍依赖已部署 CloudBase、真机或正式配置；`not-in-scope` 表示固定后台 PRD 明确排除。`implemented-local` 不代表真机或生产验收通过。
...
飞书只采纳可读取且明确的条目；未取证内容不得补造。Figma 已通过浏览器逐 frame 核对原始画布，但 MCP Starter 配额受限，不记为 design context 成功证据。
...
| H1 | 用户组合筛选、详情、名单、导出 | implemented-local | 组合筛选、用户聚合详情、脱敏/授权原文、名单和受控导出已覆盖；CloudBase 导出运行时待验收 |
| H2 | 活动 CRUD、参与人、订单、导出 | external-wait | 活动状态、报名审核、参与名单答案/时间、订单组合筛选、退款动作、相册和导出已本地实现；正式支付退款和云端导出待验收 |
| H3 | 多范围角色、敏感权限、登录审计 | implemented-local | 平台/分会/活动七类角色、窄化委派、独立敏感 capability 和管理端进入审计已覆盖 |
| H4 | 等级、权益、规则、流水、调整 | implemented-local | 等级、结构化权益、固定规则、流水、受控人工调整和业务 producer 已覆盖；正式数值待配置 |
...
| 范围 | 状态 | 原因 |
| --- | --- | --- |
| 游戏币、勋章、PK、赛季、排行榜、NPC 任务、复杂审批和独立的微信订阅消息管理模块 | not-in-scope | 后台 PRD 明确排除；D8 只保留可选投递 adapter，CSV 中只有名称或缺少规则/UI 的行不构成实现依据 |
