...
| 环境 | 当前已核实状态 | 不能外推 |
| --- | --- | --- |
| MIP staging | 2026-09-23 TEST staging 已回读 92/92 个迁移、146 张表的精确运行权限、16 个核心函数的 MySQL 健康与受保护调用规则；见[本轮部署证据](evidence/latest-requirements-2026-09-23/STAGING.md) | 不代表小程序页面、真机支付或正式生产环境通过 |
...
| 小程序运行时 | 部署前的一次 Node 22 尝试有 69/70 路由进入接受态、6 个代表状态通过；部署后的两次尝试均在首张自动截图失败并于重连超时，未形成新的路由通过数。人工看到会员、个人及机会页渲染，详见[本地验收证据](evidence/latest-requirements-2026-09-23/README.md) | 不构成 70/70 路由、43 个原型场景、完整交互或真机验收 |
