...
仓库清单当前为 66 条小程序路由、80 个锁定迁移、233 个渠道中立管理 operation（100 查询、133 写）和 16 个数据库核心函数。Web 使用其中 100 个查询与 106 个受审 mutation。以上数字只描述当前代码合同，不自动证明运行时、云端或生产通过。
...
| 管理合同 | 233 个 operation：100 查询、133 写 | `cloudfunctions/mip-admin-api/domain/public-operation-contract.js` |
