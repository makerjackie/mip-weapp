...
- 2026-09-26 评估 `weapp-vite 7.3.0`：原门禁通过，但 8 个 WXSS 消费端丢失 iOS 兼容层，故恢复 `6.25.0` 并补产物检查。候选运行尝试记录 25/70 路由（23 通过、2 失败），在身份 fixture 恢复时中断；首页加载超时、现场名单无效的原因尚未定位。详见[升级评估](../research/weapp-vite-upgrade-2026-09-26.md)，不将本次结果视为线上问题已归因或完整运行验收。
...
- J3-09 “我想合作”仍是占位提示，聚合数仍取旧引荐计数。最新画布未展开后续流程，但同一需求提交的《里程碑1+2 总览》J4-02/J4-03/W11 已定义对机会的可取消表态、只读名单和通知发布人；此前将整个功能归为“未定义”不准确。应按已有规则补实现，并统一名单可见性、历史引荐数据与“相关机会”第二页签的口径，不能直接把指定第三人的旧引荐接口改名。
- 账号设置的“会员服务协议”入口仍仅提示“功能建设中”；独立正文页与正式内容尚未交付。用户使用协议、隐私政策已有页面，不代表运营方已审定所有正文。
