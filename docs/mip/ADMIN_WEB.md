...
WorkBuddy 原型只作为管理功能、字段和信息架构输入，不作为当前交付形态或视觉基线。当前阶段不建设独立网页 UI，现有小程序管理分包将同时适配手机微信和微信电脑端：手机负责扫码、现场处理和快速操作，电脑端负责宽屏列表、筛选、表单、导出和批量处理。

两种尺寸共享同一套 MIP 用户、分会、订单、活动、权限、审计和消息事实，也共享同一套渠道中立管理合同。服务端保留未来 HTTPS/Web adapter 的接入缝隙；只有真实运营证明微信电脑端不足时，才启动独立网页后台，不提前维护第二套 UI。

## 状态定义

- `service-present`：现有服务端已经有可复用的主要业务能力。
- `service-partial`：已有部分能力，但字段、状态或行为仍有缺口。
- `web-missing`：没有网页界面、网页交互和浏览器端验收。
...
未来可选网页 UI → AdminTransport(HTTPS) ───────────────┘
```

当前不新增 Web 工程、不改成 Monorepo，也不引入 React。管理页面继续使用原生 WXML/TypeScript 和 MIP 黑黄设计系统，通过集中式响应布局原语提供手机卡片、窄窗主从和宽屏表格三种密度。WorkBuddy 的蓝白视觉不进入产品。
...
本节不是当前阶段实现范围。若后续启动独立 Web UI，推荐使用“网页展示登录码，小程序管理员确认登录”：
