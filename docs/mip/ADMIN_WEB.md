...
WorkBuddy 原型只作为管理功能、字段和信息架构输入，不作为视觉基线。小程序管理分包继续负责手机微信和微信电脑端；独立 Web 工程已经建立，用于浏览器宽屏运营。当前 7 个一级页面和 5 类详情已接入 33 条受审只读 action。
...
- 已完成：响应式概览、用户、活动、订单、权限、消息、知识库一级页面，以及用户、活动、订单、消息、知识库详情；同源 Pages Function、AES-GCM `HttpOnly` 会话、来源和请求体限制；可信 Web principal adapter；33 条由生成契约约束为安全查询的只读 action。
- 已完成安全底座：签名 envelope 的 nonce 在 MySQL 中一次性持久消费；补录会员、克隆活动、发布活动提醒和提交退款 4 条已有领域持久幂等保护的 mutation 通过精确白名单开放，网络失败不自动重试。
- 已验证：真实管理员在微信开发者工具确认网页登录后，浏览器进入 `AUTHENTICATED`；7 个一级页面以及用户、活动、订单、消息、知识库详情查询全部成功。
- 未完成：上述 4 条 mutation 的网页操作表单、其余编辑/CRUD、上传、导出。当前页面不能解释为完整网页管理能力。
...
| 用户管理 | 24 | 用户聚合、脱敏、档案、分会、角色、导出、审计为 `service-present` | `web-partial`；真实列表和聚合详情已接入，补录会员的安全服务端链路已开放；网页表单、导出和其余 CRUD 未完成，“普通用户”三分法、后台预建手机号用户和短信为 `decision-needed` |
| 活动管理 | 13 | 活动、报名、参与人、签到、反馈、相册、订单和导出为 `service-present` | `web-partial`；真实列表、详情、洞察和报名名单已接入，克隆和提醒的安全服务端链路已开放；网页表单、编辑和其余运营动作仍为 `web-missing`，支付为 `external-wait` |
...
| 订单管理 | 7 | 统一订单、支付尝试、支付 ledger、退款和脱敏导出为 `service-present` | `web-partial`；真实订单列表、汇总、筛选、分页、详情和支付尝试已接入，提交退款的安全服务端链路已开放；网页表单、导出和真实支付/回调/退款仍为 `external-wait` |
...
| 后台消息 | 6 | 站内信、运营消息活动、收件人快照、投递记录、失败复核、outbox 和 worker 为 `service-present` | `web-partial`；真实消息活动列表、详情、投递计划/记录/复核已接入，活动提醒的安全服务端链路已开放；网页表单、编辑、模板管理与正式外部投递仍未完成 |
...
独立网页 UI → AdminTransport(HTTPS；33 条只读 + 4 条受审幂等 mutation) ─┘
```
...
上述闭环目前支持 33 条受审只读 API、7 个一级页面、5 类详情和 4 条受审幂等 mutation 的服务端链路，不代表完整管理 CRUD 已完成。若业务方坚持手机号验证码/密码登录，需要额外建设验证码供应商、密码哈希、找回/重置、失败限制、异常通知和安全审计，不应作为首个版本默认方案。
