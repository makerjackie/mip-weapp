...
- 生产域名：`https://mipmini.01mvp.com/`；2026-08-28 已发布提交 `6613432`，Pages 部署为 `39971feb-2f1c-4331-9a25-4603d653f848`
...
- 线上验证：真实管理员的历史验收曾进入 `AUTHENTICATED`，覆盖 7 个一级页面以及用户、活动、订单、消息、知识库详情。当前 React 版已通过本地契约、390px/1280px/1440px 响应式和构建验证，并完成生产发布；公开入口返回 200，主资源 SHA-256 与本地 `dist` 一致，未登录同源 BFF 请求返回 `401 AUTH_REQUIRED`。
...
原型和业务方都把 MIP 城市分会称为“服务器”。Web UI 统一使用“服务器”；内部 `branch` 合同、数据库和权限范围继续使用城市分会模型。`app_id` 负责应用隔离，城市分会负责业务范围，用户档案中的公司和组织不能产生后台权限。
