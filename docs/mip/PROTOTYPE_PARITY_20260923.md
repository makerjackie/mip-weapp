...
2. **机会合作按钮**：J3-09 画布只展示入口，补充 PRD 已定义每人每机会一次、再次点击取消、只读表态名单和通知发布人。现有 `setReferral` 要求指定第三人，`setProfileInterest` 是对档案感兴趣，均不能直接改名复用。下一轮应补合作表态的数据、计数、名单与通知；需要统一的是名单权限、旧引荐数据与相关机会页签，详见[2026-09-26 复核](REVIEW_20260926.md)。
