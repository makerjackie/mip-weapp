...
| J3-09 `opp-detail-visitor` | `mip-opportunities/detail` | 类型标签、四行信息和“我想合作”按钮可见；**按钮当前仍是占位提示**。画布未展开后续流程，但同提交的《里程碑1+2 总览》J4-02/J4-03/W11 已定义表态 toggle、名单和通知。2026-09-26 复核纠正此前“整个流程未定义”的判断；旧指定对象引荐与合作表态的数据仍需迁移和统一。 |
...
2. **机会合作按钮**：J3-09 画布只展示入口，补充 PRD 已定义每人每机会一次、再次点击取消、只读表态名单和通知发布人。现有 `setReferral` 要求指定第三人，`setProfileInterest` 是对档案感兴趣，均不能直接改名复用。名单权限与个人页“我想合作”页签方向已确认，下一轮补合作表态的数据、计数、名单与通知并移除旧引荐入口；旧记录不自动转成本人表态，详见[2026-09-26 复核](REVIEW_20260926.md)。
3. **真机与外部能力**：手机号、短信、微信支付、扫码签到、分享及微信服务通知不能用开发者工具或本地测试代替。
4. **玩家等级配置与同状态实拍**：J1-06 的本地玩家账号为 Lv.1、0 项权益、1 项任务，原型为 Lv.2、4 项任务且底部为加入按钮。2026-09-26 复核确认画布已明确“2 项/4 个图标”只是示例，权益由后台配置，无需再让用户裁决这个示例数字。页面结构已修正，仍需官方等级/权益/任务配置或可复现的验收账号，再按同角色、同数据重拍；不能把原型示例数据写死在客户端。
