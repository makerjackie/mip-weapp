...
| 活动介绍媒体 | `69:4975` | `1818:17142` | `admin-web/` 的活动编辑与素材上传、`src/packages/member/mip-events/detail`（管理、展示） |
...
Figma 没有定义“更多服务”长列表或对应图片资产。`src/pages/profile` 只保留一个使用现有 token 与 TDesign 线性图标的“全部服务”入口；`src/packages/member/mip-services` 作为本地扩展二级页，按互动、工具、账号与支持分组既有真实路由，不虚构 Figma 图片或新业务能力。

Figma 也没有单独定义“账号设置”页面。`src/packages/member/privacy` 采用本地设置页结构，只保留公开设置、屏蔽列表、协议、退出登录和账号注销等可操作项；协议确认入口使用独立的 `src/packages/member/privacy-policy`，不把数据说明或订单信息混入账号设置。

## “我的”二级页设计证据

| 节点 | 设计内容 | 本地实现 |
| --- | --- | --- |
| `1774:41143` | 分组档案字段、行业/身份状态与固定保存操作 | `src/packages/member/mip-profile` 保留真实头像、昵称、姓名、性别、职业身份、分会、行业、能力、补充资料和手机号私密绑定；“更多资料”是本地渐进展开，不作为 Figma 明示要求 |
| `1732:20291` | 名片编辑、公司/职位、组织/职位和联系方式 | `src/packages/member/mip-card-edit` 编辑全部公司/组织经历和名片联系方式；手机号只通过微信授权绑定，微信号、邮箱和地址加密保存，普通公开档案接口不返回联系方式 |
| 本地扩展 | Figma 未定义独立公开范围页 | `src/packages/member/mip-visibility-settings` 统一管理公开档案和个人名片联系方式的可见性，不把公开范围混入资料填写表单 |
...
| `1725:18357`、`1725:18515` | NPC 任务列表、详情、奖励、截止与模板 | `src/packages/member/mip-tasks` 使用服务端任务状态、附件、模板和经验奖励；任务配置与派发只在 React Web 提供 |
...
| 节点 | 设计内容 | 本地实现 |
| --- | --- | --- |
| `1766:36570`、`1766:36571` | 全国选择和下拉图标 | `src/pages/opportunities/index.wxml` 顶部常驻地点入口；默认显示“全国”，应用城市或范围后同步显示当前地点，点击后进入保留搜索、城市、角色、行业和能力的筛选页 |
...
| `1766:36867`、`1766:36876`、`1766:36881` | 发布机会输入、长文本和范围控件 | 支持剪贴板整段文字的服务端 AI 结构化识别、本地字段标签解析降级、确认后填入和真实城市目录宫格；保留现有标题、价值、寻找、说明、范围、封面、标签和团队成员字段及服务端校验 |
...
- 活动介绍图片在 React Web 显示上传顺序，支持说明文字、上移、下移、移除和预览；按 Web 浏览器和真实 API 验证媒体选择、上传与失败清理。
