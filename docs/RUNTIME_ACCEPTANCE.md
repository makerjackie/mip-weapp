...
`runtime:free-event` 的受控免费活动流程通过实际页面 handler 检查必填拒绝、填写提交、修改及重新进入后的服务端版本和答案回读；没有必填字段时明确记录该项不适用。取消报名继续需要在开发者工具确认弹窗，并由活动详情回读 `CANCELLED`。此流程要求明确 TEST 环境、活动和部署证据，不能用只读路由的 ready 状态替代，也不能在未登录时判定通过。
