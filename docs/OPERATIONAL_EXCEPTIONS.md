...
异常中心是 `mip-admin-api` 提供的只读运营视图。它聚合当前 AppID 下已经持久化的异常状态，不提供重试、退款、删除素材或修改任务状态的接口。具体处置仍通过订单、活动、媒体清理和受控 worker 流程完成。
...
列表只返回来源、统一状态、固定标题和摘要、发生时间，以及经过 allowlist 生成的管理端业务路由。响应不包含 OpenID、手机号、对象存储地址、消息载荷、支付标识、错误码或 provider 返回内容。当前列表只展示最近的异常，查询最大返回 100 条，默认返回 50 条，不提供历史翻页。
