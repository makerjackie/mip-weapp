...
MIP 权威结构在 `database/mysql/mip/` 和该目录的 `migrations.lock.json`。当前 lock 固定 42 个追加迁移；新表前缀固定为 `mip_*`，完整迁移记录写入 `mip_schema_migrations`，每条语句的执行状态写入 `mip_schema_migration_steps`，引擎使用 InnoDB。`mip_orders` 统一承载会员、付费活动和付费内容订单。当前 42 个迁移已全部应用；109 张 runtime 表权限已收敛并通过幂等读回。变更前稳定备份已完成并保存在 `~/Backups/mip-weapp/2026-08-24T112700-446Z/`，本轮不再重复创建备份。
