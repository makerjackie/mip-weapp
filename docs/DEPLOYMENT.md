...
3. 对新环境或新增迁移先做仓库外逻辑备份，预览 `mip_` 迁移范围：`pnpm database:setup -- --confirm-env=<EnvID> --confirm-prefix=mip_ --dry-run`。当前共享环境的 55 个锁定迁移已经完成；只有后续新增迁移才重新进入备份、dry-run 和应用流程。
