...
const ROLE_OPTIONS: readonly AdminMutationFieldOption[] = [
  ...
  { value: 'BRANCH_ADMIN', label: '分会管理员' },
...
export const ADMIN_PEOPLE_MUTATION_CONFIG = {
  ...
  'mip.admin.users.changePrimaryBranch': {
    ...
    capability: 'users.fields.edit',
    title: '变更主分会',
    description: '为用户选择新的主分会，并记录变更原因。',
    fields: [
      { name: 'targetBranchId', label: '目标分会', kind: 'select', required: true },
  ...
  'mip.admin.roles.set': {
    ...
    fields: [
      ...
      { name: 'branchId', label: '所属分会标识', kind: 'text' },
  ...
  'mip.admin.branches.create': {
    ...
    title: '创建城市分会',
    description: '填写城市分会的标识、名称和说明。新分会创建后默认启用。',
    fields: [
      { name: 'branchKey', label: '分会标识', kind: 'text', required: true, maxLength: 64 },
      { name: 'name', label: '分会名称', kind: 'text', required: true, maxLength: 80 },
      { name: 'cityName', label: '城市名称', kind: 'text', required: true, maxLength: 80 },
      { name: 'summary', label: '分会说明', kind: 'textarea', maxLength: 500, wide: true },
    ],
  ...
  'mip.admin.branches.update': {
    ...
    capability: 'branches.manage',
    title: '更新城市分会',
    description: '更新城市分会名称、城市和说明。分会标识创建后不能修改。',
    fields: [
      { name: 'name', label: '分会名称', kind: 'text', required: true, maxLength: 80 },
      { name: 'cityName', label: '城市名称', kind: 'text', required: true, maxLength: 80 },
      { name: 'summary', label: '分会说明', kind: 'textarea', maxLength: 500, wide: true },
  ...
  'mip.admin.branches.changeStatus': {
    ...
    capability: 'branches.manage',
    title: '更新分会状态',
    description: '更新城市分会的启用状态。停用前由服务端检查关联业务记录。',
    fields: [{
      name: 'status', label: '分会状态', kind: 'select', required: true,
