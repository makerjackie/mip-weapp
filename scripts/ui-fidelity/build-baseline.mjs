...
if (!fs.existsSync(tokensPath)) {
  console.error(`missing SOT: ${tokensPath}\nInstall the mip-design-system skill before regenerating the baseline.`)
  process.exit(1)
}

const tokensSource = fs.readFileSync(tokensPath, 'utf8')
const snapshot = (manifestPath && fs.existsSync(manifestPath) ? fs.readFileSync(manifestPath, 'utf8') : '').match(/snapshot[:\s]+([0-9.]+)/i)?.[1] ?? 'unknown'
...
const doc = {
  schemaVersion: 1,
  generatedFrom: 'mip-design-system skill (assets/wechat/tokens.wxss + references/DESIGN.md)',
...
console.log(`wrote ${path.relative(root, outPath)} (${doc.colors.length} colors)`)
