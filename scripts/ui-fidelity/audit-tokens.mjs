...
const YELLOW_RE = /#[0-9a-fA-F]{3,8}\b/g
...
function contextOf(line, index) {
  ...
  const prop = before.match(/([A-Za-z][A-Za-z0-9-]*)\s*:\s*[^;:]*$/)
  if (prop) return prop[1]
  const cls = before.match(/class="([^"]*)$/)?.[1]?.match(/([a-z-]+)-\[[^\]]*$/)
  if (cls) return `class:${cls[1]}`
  const key = before.match(/"([A-Za-z0-9_]+)"\s*:\s*"?\s*$/)?.[1]
  if (key) return key
  const tailwind = before.match(/([a-zA-Z-]+)-\[[^\]]*$/)
...
function findViolations(palette, allow) {
  ...
  for (const file of files) {
    ...
    lines.forEach((line, i) => {
      if (line.includes('ui-fidelity-ignore')) return
...
function classifySeverity(file, rgb, color, context, nearest) {
  if (isYellow(color) && !nearest) return 'high'
  ...
  if (surfaceToken && bgish) return 'high'
  if (isChrome && bgish) return 'high'
  if (nearest && nearest.distance < 40) return 'medium'
  ...
  for (const file of walk(SRC)) {
    ...
    text.split('\n').forEach((line, i) => {
      for (const [, size] of line.matchAll(/font-size:\s*(\d+(?:\.\d+)?)rpx/g)) {
        ...
        if (fontOk.has(value)) findings.font.ok += 1
      ...
      for (const [, r] of line.matchAll(/border-radius:\s*(\d+(?:\.\d+)?)rpx/g)) {
        ...
        if (radiusOk.has(value) || value >= 200) findings.radius.ok += 1
...
function auditHardRules(palette) {
  ...
  for (const file of walk(SRC)) {
    ...
    text.split('\n').forEach((line, i) => {
      ...
      if (/^\s*import\s+.*from\s+['"](react|react-dom|next)['"]/.test(line)) {
...
function topOffenders(violations) {
  ...
  for (const v of violations) {
    const key = v.rgb
    const entry = map.get(key) ?? { rgb: key, count: 0, files: new Set(), suggestion: v.suggestion, severity: v.severity }
    entry.count += 1
