 ...
 *
 * Components like cooperation-role-card compute their render `view` in TS (model.ts)
...
function evalTsModule(file, resolver, cache) {
  ...
  const exportNames = [...stripped.matchAll(/^export\s+(?:async\s+)?(?:function|const|let|class)\s+(\w+)/gm)].map((m) => m[1])
  const body = code.replace(/^export\s+/gm, '')
  const fn = new Function(...Object.keys(deps), `${body}\nreturn { ${exportNames.join(', ')} }`)
  const exports = fn(...Object.values(deps))
  cache.set(file, exports)
  return exports
}

/** Loads src/components/cooperation-role-card/model.ts with its catalogs dep resolved on disk. */
export function loadCooperationCardModel(srcDir) {
  ...
  const resolver = (fromFile, spec) => {
    ...
    throw new Error(`component-model: cannot resolve '${spec}' from ${fromFile}`)
  }
  return evalTsModule(path.join(srcDir, 'components/cooperation-role-card/model.ts'), resolver, cache)
}
