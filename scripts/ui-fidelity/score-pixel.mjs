...
const REF_DIR = path.join(process.env.HOME ?? '', 'project/ame-project/mip-minip-dev/figma-restored/pages')
const CHROME = path.join(process.env.HOME ?? '', 'Library/Caches/ms-playwright/chromium_headless_shell-1187/chrome-mac/headless_shell')
const MANIFEST = path.join(root, 'config/ui-fidelity-screens.json')
const VIEWPORT = { width: 375, height: 812 }
const OUT_DIR = path.join(root, '.ui-fidelity/pixel')

function rpxToPxInClasses(value) {
  return value.replace(/(-?[\d.]+)rpx/g, (_, n) => `${(Number.parseFloat(n) / 2).toFixed(3).replace(/\.?0+$/, '')}px`)
}
...
function buildDocument({ route, fixture }) {
  ...
  const pageCss = toBrowserCss(fs.existsSync(`${base}.wxss`) ? fs.readFileSync(`${base}.wxss`, 'utf8') : '')
  const themeCss = fs.readFileSync(path.join(SRC, 'app.css'), 'utf8')
    .replace(/@import[^;]+;/g, '')
    .replace(/@source[^;]+;/g, '')
...
function screenshot(htmlFile, pngFile, height) {
  ...
  if (!fs.existsSync(pngFile)) {
    throw new Error(`screenshot failed for ${htmlFile}: ${result.stderr?.slice(0, 300)}`)
  }
}
...
function scorePair(implPng, refPng, diffPath, threshold) {
  ...
  const diffPixels = pixelmatch(a.data, b.data, diff.data, width, height, { threshold: threshold ?? 0.1, includeAA: false })
  fs.writeFileSync(diffPath, PNG.sync.write(diff))
  return { width, height, diffPixels, totalPixels: width * height, score: Number((((width * height) - diffPixels) / (width * height) * 100).toFixed(2)) }
}

export function runPixelScore({ only, outDir = OUT_DIR, quiet = false } = {}) {
  if (!fs.existsSync(CHROME)) { throw new Error(`headless chromium missing at ${CHROME}`) }
  if (!fs.existsSync(MANIFEST)) { throw new Error(`no screen manifest at ${path.relative(root, MANIFEST)} — add entries to score screens`) }
  ...
  for (const entry of manifest.screens) {
    ...
    const key = entry.key
    try {
      const html = buildDocument({ route: entry.route, fixture: entry.fixture ?? {} })
      ...
      if (!fs.existsSync(refFile)) { throw new Error(`reference missing: ${refFile}`) }
      screenshot(refFile, path.join(outDir, `${key}.ref.png`), shotHeight)

      const scale = 2
      const impl = PNG.sync.read(fs.readFileSync(path.join(outDir, `${key}.impl.png`)))
      const ref = PNG.sync.read(fs.readFileSync(path.join(outDir, `${key}.ref.png`)))
      const refCrop = crop(ref, cropTop * scale, (shotHeight - cropTop - cropBottom) * scale)
      const implCrop = crop(impl, 0, refCrop.height)
      const measured = scorePair(implCrop, refCrop, path.join(outDir, `${key}.diff.png`), entry.threshold)
      results.push({
        key,
        route: entry.route,
        ...
        crop: { topPx: cropTop, bottomPx: cropBottom, comparedHeightPx: refCrop.height / scale },
      })
      if (!quiet) { console.log(`  ${String(measured.score).padStart(6)}%  ${key}  (${measured.width}x${measured.height}, diff ${measured.diffPixels})`) }
    }
    catch (error) {
      results.push({ key, route: entry.route, error: String(error.message).slice(0, 300), score: 0, pass: false })
      if (!quiet) { console.log(`   ERROR   ${key}  ${String(error.stack ?? error.message).split('\n').slice(0, 4).join(' | ').slice(0, 300)}`) }
    }
  }
  ...
  const report = {
    ...
    caveat: 'Rendered from WXML/WXSS through a browser proxy with fixture data; not WeChat DevTools output.',
    ...
    meanScore: scored.length ? Number((scored.reduce((s, r) => s + r.score, 0) / scored.length).toFixed(2)) : null,
    results,
  }
  fs.writeFileSync(path.join(outDir, 'report.json'), `${JSON.stringify(report, null, 2)}\n`)
  if (!quiet) {
    console.log(`Layer 3 pixel: ${report.passing}/${report.scored} scored screens >= ${report.threshold}% (mean ${report.meanScore}%); ${report.screensInManifest - report.scored} unscored`)
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(import.meta.filename)) {
  ...
  const report = runPixelScore({ only: onlyIdx >= 0 ? args[onlyIdx + 1] : null })
  process.exitCode = report.scored > 0 && report.passing === report.scored ? 0 : 1
}
