...
function colorFindings(source, palette, allowlist, contextAllow = new Set()) {
  ...
  for (const line of source.split('\n')) {
    ...
    for (const match of line.matchAll(COLOR_RE)) {
      ...
      if (
        palette.index.has(rgb)
        || (parsed.a < 0.999 && palette.triples.has(triple))
      ) {
        audited += 1
      }
      else if (
        parsed.a < 0.999
        && ['0,0,0', '255,255,255'].includes(triple)
      ) {
        continue
      }
        ...
        out.push({
          ...
          rgb,
          context,
          nearest: nearestToken(parsed, palette)?.entry?.token ?? null,
        })
      }
    }
  }
...
const RADIUS_RES = [
  /border-radius:\s*(\d+)rpx/g,
  /rounded(?:-[a-z]{1,2})*-\[(\d+)rpx\]/g,
]
...
function collect(source, regexes, ok, kind) {
  ...
  return {
    total,
    off: total
      .filter(n => !ok.has(n) && n < 200)
      .map(value => ({ kind, value })),
  }
}

function geometryFindings(source) {
  return collect(source, FONT_RES, FONT_OK, 'font').off.concat(
    collect(source, RADIUS_RES, RADIUS_OK, 'radius').off,
  )
}

function countGeometryFacts(source) {
  return (
    collect(source, FONT_RES, FONT_OK, 'font').total.length
    + collect(source, RADIUS_RES, RADIUS_OK, 'radius').total.length
  )
}
...
function readIconExceptionNames() {
  const allowDoc = fs.existsSync(
    path.join(root, 'config/ui-fidelity-allowlist.json'),
  )
    ? JSON.parse(fs.readFileSync(path.join(root, 'config/ui-fidelity-allowlist.json'), 'utf8'))
    : {}
  const names = new Set()
  for (const group of allowDoc.foreignIcons ?? []) {
...
export function runScreenAudit({ threshold = 92, quiet = false } = {}) {
  ...
  const iconExceptionNames = readIconExceptionNames()
  const routes = routeList()
  const screens = []

  for (const route of routes) {
    const files = screenFiles(route)
    const pageFileExtensions = ['.wxml', '.wxss', '.css', '.ts', '.json']
    const pageFiles = new Set(pageFileExtensions.map(ext => `src/${route}${ext}`))
    const allowlist = new Set(
      (allowDoc.colors ?? [])
        .filter(entry => pageFiles.has(entry.file))
        .map(entry => formatColor(parseColor(entry.value))),
    )
    const body = `${files.wxml}\n${files.wxss}\n${files.css}\n${files.ts}`
    const colors = colorFindings(
      body,
      palette,
      allowlist,
      new Set(allowDoc.contexts ?? []),
    )
    ...
    const iconUsages = [
      ...files.wxml.matchAll(/<(mip-icon|t-icon|van-icon)\b([^>]*)>/g),
    ]
    const badIcons = iconUsages.filter(([, tag, attrs]) => {
      const name = attrs.match(/\bname="([^"{]*)"/)?.[1]
      if (tag === 'mip-icon') {
        return Boolean(name) && !iconRegistry.has(name)
      }
      return Boolean(name) && !iconExceptionNames.has(name)
    })
    const config = files.json ? JSON.parse(files.json) : {}
    const allowedBackgrounds = new Set(
      (allowDoc.pageBackgrounds ?? [])
        .filter(entry => entry.route === route)
        .map(entry => entry.value.toLowerCase()),
    )
    const pageBgOk
      = !config.navigationBarBackgroundColor
        || allowedBackgrounds.has(config.navigationBarBackgroundColor.toLowerCase())
        || config.navigationBarBackgroundColor.toLowerCase() === '#080808'
    ...
    const dimensions = {
      colors:
        colors.audited === 0
          ...
          : Number(
              (
                ((colors.audited - colors.off.length) / colors.audited)
                * 100
              ).toFixed(2),
            ),
      geometry:
        geometryFacts === 0
          ...
          : Number(
              (
                ((geometryFacts - geometry.length) / geometryFacts)
                * 100
              ).toFixed(2),
            ),
      icons:
        iconUsages.length === 0
          ...
          : Number(
              (
                ((iconUsages.length - badIcons.length) / iconUsages.length)
                * 100
              ).toFixed(2),
            ),
    ...
    screens.push({
      ...
      usesCustomComponents: [
        ...new Set(
          [
            ...files.wxml.matchAll(
              /<((?:app|mip|talent|catalog)-[a-z-]+)[\s/>]/g,
            ),
          ].map(m => m[1]),
        ),
      ],
  ...
  const report = {
    ...
    meanScore: Number(
      (screens.reduce((sum, s) => sum + s.score, 0) / screens.length).toFixed(
        2,
      ),
    ),
    layerScores: {
      tokens: layers.tokens.score,
      contracts: layers.contracts.score,
    },
    worst: screens
      .slice(0, 20)
      .map(s => ({
        route: s.route,
        score: s.score,
        colors: s.offTokenColors.length,
        geometry: s.offScaleGeometry.length,
        icons: s.offRegistryIcons,
      })),
    full: screens,
  }
  ...
  if (!quiet) {
    console.log(
      `Per-screen conformance: ${passing}/${screens.length} routes >= ${threshold}% (mean ${report.meanScore}%)`,
    )
    console.log(
      `Layer 1 tokens ${report.layerScores.tokens}% | Layer 2 contracts ${report.layerScores.contracts}%`,
    )
    for (const s of report.worst) {
      console.log(
        `  ${String(s.score).padStart(6)}%  ${s.route}  colors:${s.colors} geometry:${s.geometry} icons:${s.icons}`,
      )
    }
...
if (
  ...
  const report = runScreenAudit({
    threshold: idx >= 0 ? Number(process.argv[idx + 1]) : 92,
  })
