...
assert(!/MIP_DB_CONNECTION_URI|MIP_LEDGER_SECRET|MIP_TEST_MEMBERSHIP_HMAC_SECRET|MIP_IDENTITY_PEPPER|MIP_MEDIA_SCOPE_SECRET|MIP_PHONE_ENCRYPTION_KEY|MIP_WECHAT_APP_SECRET|MIP_ADMIN_WEB_BFF_HMAC_SECRET|MIP_ADMIN_WEB_LOGIN_HMAC_SECRET|MIP_ADMIN_WEB_LOGIN_QR_HMAC_SECRET|MIP_ADMIN_WEB_LOGIN_CONFIRM_URL/.test(clientSources), 'Server-only MIP configuration entered client source')
...
const tasksSource = sourceTree('cloudfunctions/mip-tasks-api', /\.js$/)
...
assert(adminSource.includes('MIP_TASKS_ADMIN_HMAC_SECRET')
  && adminSource.includes('mip.admin.tasks.')
  && adminSource.includes('TASKS_OPERATION_NOT_ALLOWED'), 'Task admin adapter must use an explicit signed operation boundary')
assert(tasksSource.includes('MIP_TASKS_ADMIN_HMAC_SECRET')
  && tasksSource.includes('mip-tasks-admin/v1')
  && tasksSource.includes('timingSafeEqual')
  && tasksSource.includes('admin.deleteTask'), 'Task API must authenticate and allowlist internal admin actions')
assert(adminSource.includes('MIP_BANNERS_ADMIN_HMAC_SECRET')
  && adminSource.includes('MIP_GAME_ADMIN_HMAC_SECRET')
  && adminSource.includes('MIP_MEDIA_ADMIN_HMAC_SECRET')
  && adminSource.includes('mip.admin.banners.')
  && adminSource.includes('mip.admin.game.')
  && adminSource.includes('mip.admin.media.uploadImage'), 'Admin service bridges must use explicit isolated signed operation boundaries')
assert(read('cloudfunctions/mip-banners-api/index.js').includes('MIP_BANNERS_ADMIN_HMAC_SECRET')
  && read('cloudfunctions/mip-game-api/index.js').includes('MIP_GAME_ADMIN_HMAC_SECRET')
  && read('cloudfunctions/mip-media-api/index.js').includes('MIP_MEDIA_ADMIN_HMAC_SECRET'), 'Banner, Game, and media targets must authenticate their internal admin adapters')
