...
export function buildOwnerCandidateQuery({
  ...
  const selectedUserFilter = userId ? `AND u.id = ${sqlLiteral(userId)}` : ''
  const localDemoFilter = demoUserIds.length
    ? `AND u.id NOT IN (${demoUserIds.map(sqlLiteral).join(', ')})`
    : ''
  const agreementFilters = agreements.map((agreement, index) => `AND EXISTS (
        SELECT 1
        FROM mip_agreement_acceptances agreement_${index}
        WHERE agreement_${index}.app_id = u.app_id
          AND agreement_${index}.user_id = u.id
          AND agreement_${index}.agreement_key = ${sqlLiteral(agreement.key)}
          AND agreement_${index}.agreement_version = ${sqlLiteral(agreement.version)}
      )`).join('\n      ')
  ...
  return `SELECT u.id AS candidateId
    ...
    WHERE u.app_id = ${sqlLiteral(appId)}
      AND u.status = 'ACTIVE'
      AND u.primary_branch_id IS NOT NULL
      AND CHAR_LENGTH(TRIM(profile.nickname)) > 0
      AND private_profile.phone_hash = ${sqlLiteral(phoneHash)}
