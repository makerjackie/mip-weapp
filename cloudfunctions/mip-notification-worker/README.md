...
`CHECKIN_RESULT` 使用活动参与提醒模板，逻辑字段为 `title`、`checkedAt`、`participationMethod`、`status`；`HEART_RECEIVED` 使用收到心动提醒模板，逻辑字段为 `status`、`receivedAt`。模板 ID 和关键词映射只从本地 `.env.local` 注入，不写入客户端源码。
