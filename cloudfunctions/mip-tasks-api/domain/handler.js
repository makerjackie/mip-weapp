...
const actions = Object.freeze({
  ...
  'admin.approveSubmission': (service, caller, event) => service.approveSubmission(caller, event),
  'admin.rejectSubmission': (service, caller, event) => service.rejectSubmission(caller, event),
  'admin.retrySubmissionReward': (service, caller, event) => service.retrySubmissionReward(caller, event),
  'admin.assignTask': (service, caller, event) => service.assignTask(caller, event),
  'admin.listAssignments': (service, caller, event) => service.listAssignments(caller, event),
  'admin.listTaskSubmissions': (service, caller, event) => service.listTaskSubmissions(caller, event),
  'admin.getEditorOptions': (service, caller) => service.getEditorOptions(caller),
...
const messages = Object.freeze({
  ...
  TASK_OWNER_REQUIRED: '请先关联有效的任务负责人',
  TASK_NOT_STARTED: '任务尚未开始',
  MEMBER_NOT_ELIGIBLE: '部分成员不在任务适用范围内',
  TASK_ENDED: '任务已截止',
