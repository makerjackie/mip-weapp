...
const errorMessages = {
  ...
  SMS_DISABLED: '短信验证尚未开通，可使用微信一键获取换绑',
  SMS_SEND_FAILED: '短信发送尚未确认，请稍后重试',
  SMS_RATE_LIMITED: '验证码发送过于频繁，请稍后重试',
  SMS_CODE_INVALID: '短信验证码不正确',
  SMS_CODE_EXPIRED: '短信验证码已失效，请重新获取',
  SMS_CODE_LOCKED: '验证次数过多，请重新获取验证码',
  SMS_CODE_USED: '短信验证码已使用，请刷新账号状态',
...
const actions = Object.freeze({
  ...
  requestPhoneSms: (service, caller, input) => service.requestPhoneSms(caller, { phone: input.phone }),
  bindSmsPhone: (service, caller, input) => service.bindSmsPhone(caller, { phone: input.phone, code: input.code, challengeId: input.challengeId }),
  ...
  getMembershipAgreement: (service, caller, input) => service.getMembershipAgreement(caller, { document: input.document }),
