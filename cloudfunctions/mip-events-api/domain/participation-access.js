...
function createParticipationAccessPolicy(options = {}) {
  const agreements = normalizeAgreementRequirements(options.agreements || defaultAgreements)

  async function requireAccess(queryable, appId, userId) {
    ...
    return assertParticipationAccess(accessUser(row, agreements))
  }

  return { agreements, requireAccess }
}
...
function assertParticipationAccess(user) {
