...
function connectionOptions(options = {}) {
  const shared = {
    ...
    connectionLimit: positiveInteger(options.connectionLimit || process.env.MEMBERSHIP_DB_POOL_SIZE, 4),
    queueLimit: 0,
  ...
  const uri = options.connectionUri || process.env.MEMBERSHIP_DB_CONNECTION_URI
  if (uri) {
  ...
  const host = options.host || process.env.MEMBERSHIP_DB_HOST
  const user = options.user || process.env.MEMBERSHIP_DB_USER
  const password = options.password || process.env.MEMBERSHIP_DB_PASSWORD
  const database = options.database || process.env.MEMBERSHIP_DB_NAME
  ...
  return {
    ...shared,
    host,
    port: positiveInteger(options.port || process.env.MEMBERSHIP_DB_PORT, 3306),
    user,
    password,
    database,
  }
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      ...
      try {
        ...
        try {
          ...
          console.error('[mysql] rollback failed', rollbackError?.code || 'UNKNOWN')
        }
