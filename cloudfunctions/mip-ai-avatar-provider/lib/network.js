...
function createHttpsJsonClient(options = {}) {
    ...
    try {
      records = await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = setTimeout(() => reject(new Error('timeout')), normalizeDnsTimeout(timeoutMs))
        }),
      ])
    }
    catch {
  ...
  async function postJson(endpoint, body, requestOptions = {}) {
    ...
    const maximumRequestBytes = Number(requestOptions.maximumRequestBytes || 2 * 1024 * 1024)
    const maximumResponseBytes = Number(requestOptions.maximumResponseBytes || 3 * 1024 * 1024)
    ...
    const timeoutMs = normalizeDnsTimeout(requestOptions.timeoutMs)
    ...
    return new Promise((resolve, reject) => {
      ...
      const outgoing = request(endpoint, {
        ...
        headers: {
          ...
          authorization: `Bearer ${requestOptions.authSecret}`,
        ...
        if (status >= 300 && status < 400) {
          response.resume()
          finish(new Error('DIGITAL_AVATAR_PROVIDER_REDIRECT_REJECTED'))
          return
        }
        if (status !== 200
          ...
          response.resume()
          finish(new Error('DIGITAL_AVATAR_PROVIDER_UPSTREAM_UNAVAILABLE'))
          return
        }
        const chunks = []
        let received = 0
        response.on('data', (chunk) => {
          ...
          if (received > maximumResponseBytes) {
            response.destroy()
            finish(new Error('DIGITAL_AVATAR_PROVIDER_RESPONSE_INVALID'))
            return
          }
          chunks.push(chunk)
        })
        ...
        response.on('error', () => finish(new Error('DIGITAL_AVATAR_PROVIDER_UPSTREAM_UNAVAILABLE')))
      })
      ...
      outgoing.on('error', () => finish(new Error('DIGITAL_AVATAR_PROVIDER_UPSTREAM_UNAVAILABLE')))
...
function normalizeDnsTimeout(value) {
