...
function schedulerConfig(env = process.env) {
  const functionName = functionValue(env.MIP_MESSAGE_SCHEDULER_FUNCTION_NAME, 'mip-message-scheduler')
  const adminFunctionName = functionValue(env.MIP_ADMIN_FUNCTION_NAME, 'mip-admin-api')
  const triggerName = triggerValue(env.MIP_MESSAGE_SCHEDULER_TRIGGER_NAME, 'mip-message-campaign-next')
  ...
  const secret = String(env.MIP_MESSAGE_DISPATCH_HMAC_SECRET || '')
