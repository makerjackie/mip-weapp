...
function createProfileRef({ appId, userId }, pepper) {
  if (typeof appId !== 'string' || !appId || !USER_ID_PATTERN.test(userId)) throw new Error('VALIDATION_FAILED')
...
function readProfileRef(profileRef, appId, pepper) {
  if (typeof profileRef !== 'string' || profileRef.length > 200 || !appId) throw new Error('NOT_FOUND')
  const [version, ivValue, encryptedValue, tagValue, extra] = profileRef.split('.')
  if (version !== VERSION || extra !== undefined || !ivValue || !encryptedValue || !tagValue) throw new Error('NOT_FOUND')
  try {
