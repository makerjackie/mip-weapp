...
const { stableJson } = require('./message-dispatch-auth')
...
function createMessageSchedulerClient(options = {}) {
  const now = options.now || Date.now
  const functionName = text(options.functionName) || 'mip-message-scheduler'
  ...
  return {
    configured,
    async afterSuccessfulMutation({ action, appId, mutationActions }) {
      if (!(mutationActions instanceof Set) || !mutationActions.has(action)) {
        return { status: 'SKIPPED' }
      }
      if (!configured || !text(appId)) {
...
function signSchedulerReconcile(event, secret) {
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error('MESSAGE_SCHEDULER_TIMEOUT')), timeoutMs)
        timer.unref?.()
      }),
    ])
  }
...
function boundedTimeout(value) {
  ...
  // The outer admin function has a 60s budget; keep enough room for a cold scheduler
  // and its bounded SCF/admin readbacks while still returning a retryable envelope.
...
function publicErrorCode(value) {
  ...
  return /^[A-Z][A-Z0-9_]{2,63}$/.test(code) ? code : 'MESSAGE_SCHEDULER_UNAVAILABLE'
}

function safeWarn(logger, action, error) {
  try {
    logger?.warn?.('[mip-message-scheduler-client]', {
