...
function errorResponse(error) {
  ...
  const known = {
    ...
    BANNERS_DISPATCH_CONFIG_REQUIRED: 'Banner 服务尚未配置',
    BANNERS_DISPATCH_UNAVAILABLE: 'Banner 服务暂时不可用',
    CONTENT_REJECTED: 'Banner 内容未通过安全检查，请修改后重试',
    IMAGE_ASSET_INVALID: 'Banner 图片状态无效，请重新上传',
    IMAGE_DIMENSIONS_INVALID: '图片尺寸或比例不符合要求',
    IMAGE_NOT_OWNED: '当前账号不能使用该 Banner 图片',
    TARGET_INVALID: 'Banner 跳转地址不受支持',
    MEDIA_DISPATCH_CONFIG_REQUIRED: '素材服务尚未配置',
    MEDIA_DISPATCH_UNAVAILABLE: '素材服务暂时不可用',
    PURPOSE_INVALID: '图片用途无效',
    IMAGE_INVALID: '图片内容无效',
    IMAGE_TOO_LARGE: '图片文件过大',
    IMAGE_CONTENT_REJECTED: '图片未通过内容安全检查',
    IMAGE_SAFETY_UNAVAILABLE: '图片安全检查暂时不可用',
    UPLOAD_FAILED: '图片上传失败，请稍后重试',
    GAME_DISPATCH_CONFIG_REQUIRED: '游戏服务尚未配置',
    GAME_DISPATCH_UNAVAILABLE: '游戏服务暂时不可用',
