...
function createMembershipContentRepository(database, { lockMutation, assertScope, writeAudit }) {
  async function getMembershipAgreement(appId) {
  ...
  async function saveMembershipAgreement(input) {
    return database.transaction(async tx => {
      ...
      const operation = 'membership.agreement.save'
      const claim = await claimOptional(tx, input, operation, { draft: input.draft, version: input.expectedVersion }, randomUUID)
      if (claim.replay) return claim.replay
      const row = await tx.one("SELECT version FROM mip_app_settings WHERE app_id = ? AND setting_key = 'MEMBERSHIP_AGREEMENT' FOR UPDATE", [input.appId])
      ...
      await tx.query(`INSERT INTO mip_app_settings (app_id, setting_key, value_json, updated_by_user_id)
        VALUES (?, 'MEMBERSHIP_AGREEMENT', ?, ?) ON DUPLICATE KEY UPDATE value_json = VALUES(value_json),
        updated_by_user_id = VALUES(updated_by_user_id), version = version + 1`, [input.appId, JSON.stringify(input.draft), input.actorUserId])
