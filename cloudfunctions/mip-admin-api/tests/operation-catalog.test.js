...
describe('admin operation catalog', () => {
  it('freezes all 239 business operations and keeps health outside the registry', () => {
    const catalogActions = operationCatalog.map(operation => operation.action)

    assert.equal(operationCatalog.length, 239)
    assert.equal(new Set(catalogActions).size, 239)
