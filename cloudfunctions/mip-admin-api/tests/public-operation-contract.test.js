...
describe('public admin operation contract', () => {
  it('projects all 133 business operations without exposing execution details', () => {
    assert.equal(PUBLIC_OPERATION_CONTRACT_VERSION, 1)
    assert.equal(publicOperationContract.version, 1)
    assert.equal(publicOperationContract.operationCount, 133)
    assert.equal(publicOperationContract.operations.length, 133)
  ...
  it('marks every and only query operation as safe to retry', () => {
    ...
    assert.equal(retryable.length, 53)
