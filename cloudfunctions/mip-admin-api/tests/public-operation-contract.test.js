...
describe('public admin operation contract', () => {
  it('projects all 231 business operations without exposing execution details', () => {
    assert.equal(PUBLIC_OPERATION_CONTRACT_VERSION, 1)
    assert.equal(publicOperationContract.version, 1)
    assert.equal(publicOperationContract.operationCount, 231)
    assert.equal(publicOperationContract.operations.length, 231)
  ...
  it('marks every and only query operation as safe to retry', () => {
    ...
    assert.equal(retryable.length, 99)
  ...
  it('owns the complete Web exposure, input-key, route, and idempotency policy', () => {
    ...
    assert.equal(adminWebOperationContract.operationCount, 231)
    assert.equal(adminWebOperationContract.operations.length, 231)
    ...
    assert.equal(queries.length, 99)
    assert.equal(mutations.length, 105)
