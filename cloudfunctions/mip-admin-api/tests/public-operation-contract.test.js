...
describe('public admin operation contract', () => {
  it('projects all 239 business operations without exposing execution details', () => {
    assert.equal(PUBLIC_OPERATION_CONTRACT_VERSION, 1)
    assert.equal(publicOperationContract.version, 1)
    assert.equal(publicOperationContract.operationCount, 239)
    assert.equal(publicOperationContract.operations.length, 239)
  ...
  it('marks every and only query operation as safe to retry', () => {
    ...
    assert.equal(retryable.length, 104)
  ...
  it('owns the complete Web exposure, input-key, route, and idempotency policy', () => {
    ...
    assert.equal(adminWebOperationContract.operationCount, 239)
    assert.equal(adminWebOperationContract.operations.length, 239)
    ...
    assert.equal(queries.length, 104)
    assert.equal(mutations.length, 111)
    assert.equal(blocked.length, 24)
    assert.deepEqual(
