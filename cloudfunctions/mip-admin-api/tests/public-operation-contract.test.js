...
describe('public admin operation contract', () => {
  it('projects all 144 business operations without exposing execution details', () => {
    assert.equal(PUBLIC_OPERATION_CONTRACT_VERSION, 1)
    assert.equal(publicOperationContract.version, 1)
    assert.equal(publicOperationContract.operationCount, 144)
    assert.equal(publicOperationContract.operations.length, 144)
  ...
  it('marks every and only query operation as safe to retry', () => {
    ...
    assert.equal(retryable.length, 61)
