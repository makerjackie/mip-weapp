...
const { createKnowledgeScheduler } = require('./domain/scheduler')
...
function createRuntime(context) {
  ...
  return { config, scheduler: createKnowledgeScheduler({ admin, config, trigger }) }
}
  ...
  if (event?.action === RECONCILE_ACTION) {
    try {
      ...
      return {
        ok: false,
        error: {
          code: error?.message === 'FORBIDDEN' ? 'FORBIDDEN' : 'KNOWLEDGE_SCHEDULER_UNAVAILABLE',
  ...
  if (event?.action === ACTIVATE_ACTION) {
    try {
      ...
      return {
        ok: false,
        error: {
          code: error?.message === 'FORBIDDEN' ? 'FORBIDDEN' : 'KNOWLEDGE_SCHEDULER_ACTIVATION_UNVERIFIED',
...
function safeWarn(error, mode) {
  try {
    console.warn('[mip-knowledge-scheduler]', {
