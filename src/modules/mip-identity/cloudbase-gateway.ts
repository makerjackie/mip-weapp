...
import { measureLoading, recordLoadingFailure } from '../../platform/cloudbase/loading-diagnostics'
...
export function createMipIdentityCloudbaseTransport(
  ...
  return {
    async invoke(request) {
      ...
      try {
        const result = await measureLoading('identity.request', () => retryTransport(async () => {
        ...
        if (envelope && typeof envelope === 'object' && 'ok' in envelope && envelope.ok === false) {
          recordLoadingFailure('identity.response', 'error' in envelope ? envelope.error : undefined)
        }
        return resolveCloudFileUrls(envelope, result.cloud)
      }
      catch {
