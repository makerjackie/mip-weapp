...
const readActions = new Set(['getAnnouncement', 'getRelationship', 'listAnnouncements', 'listBlocked'])

function unwrap<T>(value: unknown): T {
  if (!value || typeof value !== 'object' || typeof (value as Envelope<T>).ok !== 'boolean') {
    throw new Error('社区服务返回了无效响应')
  }
  const envelope = value as Envelope<T>
  if (!envelope.ok) {
    throw new Error(envelope.error?.message || '社区服务请求失败')
  }
...
export async function callCommunityApi<T>(action: string, data: Record<string, unknown> = {}) {
  try {
    ...
    throw new Error('社区服务暂时不可用，请稍后重试')
  }
}
